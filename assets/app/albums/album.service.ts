...
export class AlbumService
{
	...
	find(query)
	{
		var getUrl = 'http://localhost:3000/albums/find?query=' + query;
	...
	{
		var url = 'http://localhost:3000/albums/loadForArtist?artistId=' + artist._id;
	...
	loadRecent()
	{
		return this.http.get('http://localhost:3000/albums/recent')
			.map((response: Response) => {
				const data = response.json().albums;
